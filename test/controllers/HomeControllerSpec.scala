...
//        val controller = inject[HomeController]
//        val vehicle = controller.vehicle("BMW").apply(FakeRequest(GET, "/vehicle/:vehicleName"))
//
//        when(wsRequest.get())
...
//        contentAsString(vehicle) must include ("Welcome to Vehicle page")


        lazy val controller = new HomeController(wsMock, stubControllerComponents(), executionContext)
        lazy val vehicle = controller.vehicle("BMW").apply(FakeRequest(GET, "/vehicle"))
      ...
      "Failed to render the vehicle page from the application" in {
        val controller = inject[HomeController]
        val vehicle = controller.vehicle("test").apply(FakeRequest(GET, "/vehicle/:vehicleName"))
...
//        status(home) mustBe OK
//        contentType(home) mustBe Some("text/html")
//        contentAsString(home) must include ("Welcome to Vehicle page")
//      }
    }
}
